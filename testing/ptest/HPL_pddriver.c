...
int main(int ARGC, char** ARGV) {
  ...
  HPL_pdinfo(&test, &ns, nval, &nbs, nbval, &pmapping, &npqs, pval, qval, &npfs,
             ...
             &ndhs, ndhval, &fswap, &tswap, &L1notran, &Unotran, &equil,
             &align);
  /*
  ...
  for (ipq = 0; ipq < npqs; ipq++) {
    (void)HPL_grid_init(MPI_COMM_WORLD, pmapping, pval[ipq], qval[ipq], &grid);
          ...
          for (itop = 0; itop < ntps;
            ...
            for (irfa = 0; irfa < nrfs;
              ...
              for (ipfa = 0; ipfa < npfs;
                ...
                for (inbm = 0;
                     inbm < nbms; inbm++) { /* Loop over various recursive
                                               stopping criteria */
