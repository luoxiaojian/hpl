...
void HPL_pdmxswp(HPL_T_panel* PANEL, const int M, const int II, const int JJ,
                 double* WORK) {
  /*
   * Purpose
   * =======
  ...
  if ((Np2 != 0) && ((partner = (int)((unsigned int)(mydist) ^ ip2)) < nprow)) {
    if ((mydist & ip2) != 0) {
      if (mydist == (int)(ip2))
        (void)HPL_sdrv(WORK, cnt_, MSGID_BEGIN_PFACT, A0, n0, MSGID_BEGIN_PFACT,
                       MModAdd(partner, icurrow, nprow), comm);
      else
        (void)HPL_send(WORK, cnt_, MModAdd(partner, icurrow, nprow),
                       MSGID_BEGIN_PFACT, comm);
    } else {
      if (mydist == 0)
        (void)HPL_sdrv(A0, n0, MSGID_BEGIN_PFACT, Wwork, cnt_,
                       MSGID_BEGIN_PFACT, MModAdd(partner, icurrow, nprow),
                       comm);
      else
        (void)HPL_recv(Wwork, cnt_, MModAdd(partner, icurrow, nprow),
                       MSGID_BEGIN_PFACT, comm);
  ...
  if (mydist < (int)(ip2)) {
    ...
    while (k < hdim) {
      ...
      gmax = Mabs(WORK[0]);
      if ((tmp1 > gmax) || ((tmp1 == gmax) && (Wwork[3] < WORK[3]))) {
        HPL_dcopy((rcnt == cnt0 ? cnt0 : cnt_), Wwork, 1, WORK, 1);
      } else if (rcnt == cnt0) {
    ...
    do {
      ...
      if ((mydis_ & mask) == 0) {
        ...
        if ((mydis_ & ip2_) != 0) {
          (void)HPL_recv(A0, n0, MModAdd(root, partner, nprow),
                         MSGID_BEGIN_PFACT, comm);
        } else if (partner < size_) {
          (void)HPL_send(A0, n0, MModAdd(root, partner, nprow),
                         MSGID_BEGIN_PFACT, comm);
        }
      }
      ip2_ >>= 1;
