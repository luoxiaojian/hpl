...
void HPL_pdpanrlT(HPL_T_panel* PANEL, const int M, const int N, const int ICOFF,
                  double* WORK) {
  /*
   * Purpose
   * =======
  ...
  while (Nm1 >= 1) {
    ...
    if (Nm1 > 1) {
       ...
       */
      Av1 = vsip_msubview_d(Av0, PANEL->ii + iip1, PANEL->jj + jj + 2, Mm1,
                            Nm1 - 1);
      Xv1 = vsip_msubview_d(Av0, PANEL->ii + iip1, PANEL->jj + jj, Mm1, 1);
      ...
      HPL_dger(HplColumnMajor, Mm1, Nm1 - 1, -HPL_rone, Acur, 1,
               Mptr(L1, jj + 2, jj, n0), 1, Mptr(Anxt, 0, 1, lda), lda);
#endif
    }
    if (curr != 0) {
      ii = iip1;
