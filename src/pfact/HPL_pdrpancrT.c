 ...
 */
    ...
    if (curr != 0) {
      Av1 = vsip_msubview_d(Av0, PANEL->ii + ICOFF + ii, PANEL->jj + ICOFF, m,
                            jj);
      Av2 =
          vsip_msubview_d(Av0, PANEL->ii + ICOFF + ii, PANEL->jj + ioff, m, jb);
    } else {
    ...
    vsip_gemp_d(-HPL_rone, Av1, VSIP_MAT_NTRANS, Lv1, VSIP_MAT_TRANS, HPL_rone,
                Av2);
    /*
...
#else
    HPL_dgemm(HplColumnMajor, HplNoTrans, HplTrans, m, jb, jj, -HPL_rone,
              Mptr(Aptr, ii, 0, lda), lda, Mptr(L1ptr, jj, 0, n0), n0, HPL_rone,
    ...
    if (n > 0) {
      ...
      (void)vsip_mdestroy_d(Lv0);
#else
      HPL_dgemm(HplColumnMajor, HplNoTrans, HplNoTrans, n, jb, jj, -HPL_rone,
                Mptr(L1ptr, jj + jb, 0, n0), n0, Mptr(L1ptr, 0, jj, n0), n0,
                HPL_rone, Mptr(L1ptr, jj + jb, jj, n0), n0);
#endif
      HPL_dtrsm(HplColumnMajor, HplRight, HplUpper, HplNoTrans, HplUnit, n, jb,
                HPL_rone, Mptr(L1ptr, jj, jj, n0), n0,
     ...
     */
    if (curr != 0) {
      HPL_dlatcpy(ioff, jb, Mptr(L1, ioff, 0, n0), n0, Mptr(A, 0, ioff, lda),
                  lda);
      ii += jb;
      m -= jb;
    }
