...
void HPL_pdpancrT(HPL_T_panel* PANEL, const int M, const int N, const int ICOFF,
                  double* WORK) {
  /*
   * Purpose
   * =======
  ...
  while (Nm1 > 0) {
    ...
    if (kk > 0) {
      ...
      HPL_dgemv(HplColumnMajor, HplNoTrans, Nm1, kk, -HPL_rone,
                Mptr(L1, jj + 1, ICOFF, n0), n0, Mptr(L1, ICOFF, jj, n0), 1,
                HPL_rone, L1ptr, 1);
#endif
    ...
    vsip_gemp_d(-HPL_rone, Av1, VSIP_MAT_NTRANS, Xv1, VSIP_MAT_TRANS, HPL_rone,
                Yv1);
    /*
     * Destroy the matrix subviews
     */
    ...
    HPL_dgemv(HplColumnMajor, HplNoTrans, Mm1, kk + 1, -HPL_rone,
              Mptr(A, iip1, ICOFF, lda), lda, Mptr(L1, jj + 1, ICOFF, n0), n0,
