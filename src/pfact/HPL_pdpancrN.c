...
void HPL_pdpancrN(HPL_T_panel* PANEL, const int M, const int N, const int ICOFF,
                  double* WORK) {
  /*
   * Purpose
   * =======
  ...
  while (Nm1 > 0) {
    ...
    if (kk > 0) {
      ...
      HPL_dgemv(HplColumnMajor, HplTrans, kk, Nm1, -HPL_rone,
                Mptr(L1, ICOFF, jj + 1, n0), n0, Mptr(L1, jj, ICOFF, n0), n0,
                HPL_rone, L1ptr, n0);
#endif
     ...
     */
    Av1 =
        vsip_msubview_d(Av0, PANEL->ii + iip1, PANEL->jj + ICOFF, Mm1, kk + 1);
    Xv1 = vsip_msubview_d(Xv0, ICOFF, jj + 1, kk + 1, 1);
    Yv1 = vsip_msubview_d(Av0, PANEL->ii + iip1, PANEL->jj + jj + 1, Mm1, 1);

    vsip_gemp_d(-HPL_rone, Av1, VSIP_MAT_NTRANS, Xv1, VSIP_MAT_NTRANS, HPL_rone,
                Yv1);
    /*
     * Destroy the matrix subviews
     */
    ...
    HPL_dgemv(HplColumnMajor, HplNoTrans, Mm1, kk + 1, -HPL_rone,
              Mptr(A, iip1, ICOFF, lda), lda, Mptr(L1, ICOFF, jj + 1, n0), 1,
