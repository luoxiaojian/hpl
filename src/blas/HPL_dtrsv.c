...
static void HPL_dtrsvLNN(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
...
static void HPL_dtrsvLNU(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx, ldap1 = LDA + 1;
  register double t0;
...
static void HPL_dtrsvLTN(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx, ldap1 = LDA + 1;
  register double t0;

  for (j = N - 1, jaj = (N - 1) * (ldap1), jx = (N - 1) * INCX; j >= 0;
       j--, jaj -= ldap1, jx -= INCX) {
    t0 = X[jx];
    for (i = j + 1, iaij = 1 + jaj, ix = jx + INCX; i < N;
         i++, iaij += 1, ix += INCX) {
      t0 -= A[iaij] * X[ix];
    }
    t0 /= A[jaj];
...
static void HPL_dtrsvLTU(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx, ldap1 = LDA + 1;
  register double t0;
...
static void HPL_dtrsvUNN(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx;
  register double t0;

  for (j = N - 1, jaj = (N - 1) * LDA, jx = (N - 1) * INCX; j >= 0;
       j--, jaj -= LDA, jx -= INCX) {
    X[jx] /= A[j + jaj];
    t0 = X[jx];
...
static void HPL_dtrsvUNU(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx;
  register double t0;
...
static void HPL_dtrsvUTN(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx;
  register double t0;
...
static void HPL_dtrsvUTU(const int N, const double* A, const int LDA, double* X,
                         const int INCX) {
  int i, iaij, ix, j, jaj, jx;
  register double t0;
...
static void HPL_dtrsv0(const enum HPL_UPLO UPLO, const enum HPL_TRANS TRANS,
                       const enum HPL_DIAG DIAG, const int N, const double* A,
  ...
  if (UPLO == HplUpper) {
    if (TRANS == HplNoTrans) {
      if (DIAG == HplNonUnit) {
        HPL_dtrsvUNN(N, A, LDA, X, INCX);
      } else {
        HPL_dtrsvUNU(N, A, LDA, X, INCX);
      }
    } else {
      if (DIAG == HplNonUnit) {
        HPL_dtrsvUTN(N, A, LDA, X, INCX);
      } else {
        HPL_dtrsvUTU(N, A, LDA, X, INCX);
      }
    }
  } else {
    if (TRANS == HplNoTrans) {
      if (DIAG == HplNonUnit) {
        HPL_dtrsvLNN(N, A, LDA, X, INCX);
      } else {
        HPL_dtrsvLNU(N, A, LDA, X, INCX);
      }
    } else {
      if (DIAG == HplNonUnit) {
        HPL_dtrsvLTN(N, A, LDA, X, INCX);
      } else {
        HPL_dtrsvLTU(N, A, LDA, X, INCX);
      }
    }
  }
}
 ...
 */
  ...
  if (ORDER == HplColumnMajor) {
    ...
    HPL_dtrsv0((UPLO == HplUpper ? HplLower : HplUpper),
               (TRANS == HplNoTrans ? HplTrans : HplNoTrans), DIAG, N, A, LDA,
               X, INCX);
  }
#endif
...
#ifdef StringSunStyle
  F77dtrsv(&cuplo, &ctran, &cdiag, &F77N, A, &F77lda, X, &F77incx, IONE, IONE,
           IONE);
#endif
#ifdef StringCrayStyle
