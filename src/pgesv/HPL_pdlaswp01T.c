...
void HPL_pdlaswp01T(HPL_T_panel* PBCST, int* IFLAG, HPL_T_panel* PANEL,
                    const int NN) {
  /*
   * Purpose
   * =======
...
#define LDU n
  ...
  } else if (*iflag == 0) /* HPL_pdlaswp00T called before: reuse ipID */
  {
    ...
    *iflag = 1;
             ...
             (equil != 0)) { /* HPL_pdlaswp01T was call before only re-compute
                                IPLEN, IPMAP */
   ...
   */
  HPL_spreadT(PBCST, IFLAG, PANEL, HplRight, n, U, LDU, 0, iplen, ipmap,
              ipmapm1);
  /*
  ...
  if (myrow != icurrow) {
    k = ipmapm1[myrow];
    HPL_dlaswp06T(iplen[k + 1] - iplen[k], n, A, lda, Mptr(U, 0, iplen[k], LDU),
                  LDU, lindxA);
  }
  /*
   * Equilibration
   */
  if (equil != 0)
    HPL_equil(PBCST, IFLAG, PANEL, HplTrans, n, U, LDU, iplen, ipmap, ipmapm1,
              iwork);
  /*
   * Rolling phase
   */
