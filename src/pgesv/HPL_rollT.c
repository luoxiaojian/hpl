...
void HPL_rollT(HPL_T_panel* PBCST, int* IFLAG, HPL_T_panel* PANEL, const int N,
 ...
 */
  ...
  for (k = 0; k < npm1; k++) {
    ...
    if (lengthR > 0) {
       ...
       */
      if (ierr == MPI_SUCCESS)
        ierr = MPI_Irecv(Mptr(U, 0, ibufR, LDU), lengthR * LDU, MPI_DOUBLE,
                         partner, Cmsgid, comm, &request);
#endif
    }
    ...
    if (lengthS > 0) {
       ...
       */
      if (ierr == MPI_SUCCESS)
        ierr = MPI_Send(Mptr(U, 0, ibufS, LDU), lengthS * LDU, MPI_DOUBLE,
                        partner, Cmsgid, comm);
#endif
    }
