...
void HPL_pdtrsv(HPL_T_grid *GRID, HPL_T_pmat *AMAT) {
  ...
  if (mycol == Alcol) {
    ...
    Xdprev = (Xd = XR + Anq);
    if (myrow == Alrow) {
      HPL_dtrsv(HplColumnMajor, HplUpper, HplNoTrans, HplNonUnit, kb,
                Aptr + Anp, lda, XC + Anp, 1);
  ...
  while (n > 0) {
    ...
    if (mycol == colprev) {
      ...
      if (n1pprev > 0) {
        ...
        HPL_dgemv(HplColumnMajor, HplNoTrans, n1pprev, kbprev, -HPL_rone,
    ...
    */
    if ((mycol == colprev) && ((tmp1 = Anpprev - n1pprev) > 0))
      HPL_dgemv(HplColumnMajor, HplNoTrans, tmp1, kbprev, -HPL_rone, Aprev, lda,
                Xdprev, 1, HPL_rone, XC, 1);
    /*
