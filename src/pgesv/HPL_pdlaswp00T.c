...
void HPL_pdlaswp00T(HPL_T_panel *PBCST, int *IFLAG, HPL_T_panel *PANEL,
                    const int NN) {
  /*
   * Purpose
   * =======
    ...
    while (k < hdim) {
      ...
      if ((mydist >> (unsigned int)(k + 1)) == 0) {
        if ((mydist >> (unsigned int)(k)) == 0) {
          ...
          HPL_dlaswp03T(llen[partner], n, U, LDU, Mptr(W, 0, ipW, ldW),
                        Mptr(W, 1, ipW, ldW), ldW);
          ipW += llen[partner];
        } else {
          (void)HPL_sdrv(W, llen[myrow] * ldW, Cmsgid, U, usize, Cmsgid,
                         partner, comm);
          HPL_dlaswp04T(ipA, llen[myrow], n, U, LDU, A, lda, W, W + 1, ldW,
                        lindxA, lindxAU);
        }
      } else {
