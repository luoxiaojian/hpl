...
void HPL_spreadT(HPL_T_panel* PBCST, int* IFLAG, HPL_T_panel* PANEL,
      ...
      if ((mydist & mask) == 0) {
        ...
        if (lbuf > 0) {
          ...
          if (mydist & ip2) {
             ...
             */
            if (ierr == MPI_SUCCESS)
              ierr = MPI_Recv(Mptr(U, 0, ibuf, LDU), lbuf * N, MPI_DOUBLE,
             ...
             */
            if (ierr == MPI_SUCCESS)
              ierr = MPI_Send(Mptr(U, 0, ibuf, LDU), lbuf * N, MPI_DOUBLE,
                              IPMAP[npm1 - partner], Cmsgid, comm);
#endif
          }
        }
