...
void HPL_pdupdateNT(HPL_T_panel* PBCST, int* IFLAG, HPL_T_panel* PANEL,
                    const int NN) {
  /*
   * Purpose
   * =======
 ...
 */
      ...
      HPL_dlaswp00N(jb, nn, Aptr, lda, ipiv);
#endif
      HPL_dtrsm(HplColumnMajor, HplLeft, HplLower, HplNoTrans, HplUnit, jb, nn,
                HPL_rone, L1ptr, jb, Aptr, lda);
#ifdef HPL_CALL_VSIPL
      /*
       * Create the matrix subviews
       */
...
#else
      HPL_dgemm(HplColumnMajor, HplNoTrans, HplNoTrans, mp, nn, jb, -HPL_rone,
                L2ptr, ldl2, Aptr, lda, HPL_rone, Mptr(Aptr, jb, 0, lda), lda);
#endif
  ...
  } else /* nprow > 1 ... */
  {
    ...
    while (test == HPL_KEEP_TESTING) {
      ...
      HPL_dtrsm(HplColumnMajor, HplRight, HplLower, HplTrans, HplUnit, nn, jb,
                HPL_rone, L1ptr, jb, Uptr, LDU);
      ...
      if (curr != 0) {
        ...
        HPL_dgemm(HplColumnMajor, HplNoTrans, HplTrans, mp, nn, jb, -HPL_rone,
                  L2ptr, ldl2, Uptr, LDU, HPL_rone, Mptr(Aptr, jb, 0, lda),
                  lda);
#endif
...
#else
        HPL_dgemm(HplColumnMajor, HplNoTrans, HplTrans, mp, nn, jb, -HPL_rone,
                  L2ptr, ldl2, Uptr, LDU, HPL_rone, Aptr, lda);
#endif
      }
