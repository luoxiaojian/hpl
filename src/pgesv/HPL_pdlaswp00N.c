...
void HPL_pdlaswp00N(HPL_T_panel *PBCST, int *IFLAG, HPL_T_panel *PANEL,
                    const int NN) {
  /*
   * Purpose
   * =======
  ...
  double *A, *U, *W;
  void *vptr = NULL;
  int *ipID, *lindxA, *lindxAU, *llen, *llen_sv;
  unsigned int ip2, ip2_ = 1, ipdist, ipow = 1, mask = 1, mydist, mydis_;
  int Cmsgid = MSGID_BEGIN_PFACT, Np2, align, hdim, i, icurrow, *iflag, ipA,
      ipW, *ipl, iprow, jb, k, lda, ldW, myrow, n, nprow, partner, root, size_,
      usize;
#define LDU jb
  n = Mmin(NN, PANEL->n);
  jb = PANEL->jb;
   ...
   */
  vptr = (void *)malloc(((size_t)(align) + ((size_t)(jb) * (size_t)(ldW))) *
                        sizeof(double));
  if (vptr == NULL) {
  ...
  if ((Np2 != 0) && ((partner = (int)(mydist ^ ip2)) < nprow)) {
    ...
    {
      (void)HPL_sdrv(U, usize, Cmsgid, W, llen[partner] * ldW, Cmsgid, partner,
                     comm);
      if (llen[partner] > 0)
        HPL_dlaswp03N(llen[partner], n, U, LDU, W, W + 1, ldW);
    } else if (mydist == ip2) { /* I recv U for later Bcast, I send my W */
      (void)HPL_sdrv(W, llen[myrow] * ldW, Cmsgid, U, usize, Cmsgid, partner,
                     comm);
    } else /* None of us is icurrow, we exchange our Ws */
    {
      if ((mydist & ip2) != 0) {
        ...
        (void)HPL_recv(Mptr(W, 0, ipW, ldW), llen[partner] * ldW, partner,
                       Cmsgid, comm);
    ...
    while (k < hdim) {
      ...
      if ((mydist >> (unsigned int)(k + 1)) == 0) {
        if ((mydist >> (unsigned int)(k)) == 0) {
          (void)HPL_sdrv(U, usize, Cmsgid, Mptr(W, 0, ipW, ldW),
                         llen[partner] * ldW, Cmsgid, partner, comm);
          HPL_dlaswp03N(llen[partner], n, U, LDU, Mptr(W, 0, ipW, ldW),
                        Mptr(W, 1, ipW, ldW), ldW);
          ipW += llen[partner];
        } else {
          (void)HPL_sdrv(W, llen[myrow] * ldW, Cmsgid, U, usize, Cmsgid,
                         partner, comm);
          HPL_dlaswp04N(ipA, llen[myrow], n, U, LDU, A, lda, W, W + 1, ldW,
                        lindxA, lindxAU);
        }
      } else {
        (void)HPL_sdrv(W, llen[myrow] * ldW, Cmsgid, Mptr(W, 0, ipW, ldW),
                       llen[partner] * ldW, Cmsgid, partner, comm);
        ipW += llen[partner];
      }
      /*
    ...
    if (size_ > 1) {
      ...
      do {
        ...
        if ((mydis_ & mask) == 0) {
          ...
          if ((mydis_ & ip2_) != 0) {
            (void)HPL_recv(U, usize, MModAdd(root, partner, nprow), Cmsgid,
                           comm);

          } else if (partner < size_) {
            (void)HPL_send(U, usize, MModAdd(root, partner, nprow), Cmsgid,
                           comm);
          }
        }
        ip2_ >>= 1;
