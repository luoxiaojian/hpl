...
int HPL_bcast_blonM(HPL_T_panel *PANEL, int *IFLAG) {
  ...
  if (rank == root) {
#ifdef HPL_USE_MPI_DATATYPE
    if (ierr == MPI_SUCCESS) ierr = HPL_packL(PANEL, 0, PANEL->len, I_SEND);
#endif
    if (ierr == MPI_SUCCESS)
      ierr = MPI_Ssend(_M_BUFF_S1, _M_COUNT_S1, _M_TYPE_S1,
                       MModAdd1(rank, size), msgid, comm);
#ifdef HPL_USE_MPI_DATATYPE
    if (ierr == MPI_SUCCESS) ierr = MPI_Type_free(&PANEL->dtypes[I_SEND]);
#endif
  } else if (prev == root) {
    /*
...
#endif
        if (ierr == MPI_SUCCESS)
          ierr = MPI_Recv(_M_BUFF_R1, _M_COUNT_R1, _M_TYPE_R1, root, msgid,
                          comm, &PANEL->status[0]);
#ifdef HPL_USE_MPI_DATATYPE
        if (ierr == MPI_SUCCESS) ierr = MPI_Type_free(&PANEL->dtypes[I_RECV]);
#endif
      } else {
 ...
 */
        ...
        if (lbuf > 0) {
#ifdef HPL_USE_MPI_DATATYPE
          if (ierr == MPI_SUCCESS) ierr = HPL_packL(PANEL, ibuf, lbuf, I_RECV);
#endif
          if (ierr == MPI_SUCCESS)
            ierr = MPI_Recv(_M_BUFF_R2, _M_COUNT_R2, _M_TYPE_R2, partner, msgid,
                            comm, &PANEL->status[0]);
#ifdef HPL_USE_MPI_DATATYPE
          if (ierr == MPI_SUCCESS) ierr = MPI_Type_free(&PANEL->dtypes[I_RECV]);
#endif
        ...
        {
          if (ierr == MPI_SUCCESS)
            ierr = MPI_Recv((void *)(&dummy), 0, MPI_BYTE, partner, msgid, comm,
                            &PANEL->status[0]);
        }
        ...
        if (lbuf > 0) {
#ifdef HPL_USE_MPI_DATATYPE
          if (ierr == MPI_SUCCESS) ierr = HPL_packL(PANEL, ibuf, lbuf, I_SEND);
#endif
          if (ierr == MPI_SUCCESS)
            ierr = MPI_Ssend(_M_BUFF_S2, _M_COUNT_S2, _M_TYPE_S2, partner,
                             msgid, comm);
#ifdef HPL_USE_MPI_DATATYPE
          if (ierr == MPI_SUCCESS) ierr = MPI_Type_free(&PANEL->dtypes[I_SEND]);
#endif
  ...
  for (k = 0; k < npm2; k++) {
    ...
    if (lbufS > 0) {
#ifdef HPL_USE_MPI_DATATYPE
      if (ierr == MPI_SUCCESS) ierr = HPL_packL(PANEL, ibufS, lbufS, I_SEND);
#endif
      if (ierr == MPI_SUCCESS)
        ierr = MPI_Issend(_M_ROLL_BUFF_S, _M_ROLL_COUNT_S, _M_ROLL_TYPE_S,
                          partner, msgid, comm, &PANEL->request[0]);
    } else {
      if (ierr == MPI_SUCCESS)
        ierr = MPI_Issend((void *)(&dummy), 0, MPI_BYTE, partner, msgid, comm,
                          &PANEL->request[0]);
    }

    if (lbufR > 0) {
#ifdef HPL_USE_MPI_DATATYPE
      if (ierr == MPI_SUCCESS) ierr = HPL_packL(PANEL, ibufR, lbufR, I_RECV);
#endif
      if (ierr == MPI_SUCCESS)
        ierr = MPI_Recv(_M_ROLL_BUFF_R, _M_ROLL_COUNT_R, _M_ROLL_TYPE_R,
