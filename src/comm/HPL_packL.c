 ...
 */
  ...
  int *blen = NULL;
  MPI_Aint *disp = NULL;
  int curr, i, i1, ibuf, ierr = MPI_SUCCESS, j1, jb, jbm, jbp1, lda, len, m, m1,
                         nbufs;
#else
  int ierr;
#endif
  ...
  if (ierr == MPI_SUCCESS) ierr = MPI_Type_commit(&PANEL->dtypes[IBUF]);
  ...
  if ((bufs != NULL) && (blen != NULL) && (disp != NULL) && (type != NULL)) {
    m = PANEL->mp;
    ...
    if ((m > 0) && (ibuf < jbm)) {
      ...
      blen[nbufs] = m1;
      if (ierr == MPI_SUCCESS) ierr = MPI_Address(bufs[nbufs], &disp[nbufs]);
    ...
    for (i = 1; i < nbufs; i++) disp[i] -= disp[0];
    disp[0] = 0;
     ...
     */
    if (ierr == MPI_SUCCESS)
      ierr = MPI_Type_struct(nbufs, blen, disp, type, &PANEL->dtypes[IBUF]);
    /*
     * release temporaries
     */
    if (bufs) free(bufs);
